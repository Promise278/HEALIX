...
const Chat = () => {
  ...
  const fetchConversations = async () => {
    try {
      ...
      const response = await fetch(`${Config.API_URL}/api/messages/conversations/${user.id}`, {
        headers: {
  ...
  return (
    ...
    >
      ...
      <View className="gap-3">
        {loading ? (
          <ActivityIndicator size="large" color="#23a9ba" className="mt-10" />
              ...
              <View className="flex-1">
                <View className="flex-row justify-between items-start mb-1">
                  ...
                  <Text className="text-xs text-gray-400 ml-2">
                    {chat.timestamp ? new Date(chat.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "Recently"}
                  </Text>
                </View>
